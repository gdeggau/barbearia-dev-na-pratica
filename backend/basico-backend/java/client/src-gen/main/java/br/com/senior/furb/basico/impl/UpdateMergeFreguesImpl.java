 ...
 * 
 */
public class UpdateMergeFreguesImpl {
	...
	public UpdateMergeFreguesImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Fregues input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.UPDATE_MERGE_FREGUES, //
	...
	*/
	public Fregues updateMergeFregues(Fregues input, long timeout) {
		...
		Fregues output = DtoJsonConverter.toDTO(resultMessage.getContent(), Fregues.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Fregues> updateMergeFreguesRequest(Fregues input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Fregues.class);
	}
