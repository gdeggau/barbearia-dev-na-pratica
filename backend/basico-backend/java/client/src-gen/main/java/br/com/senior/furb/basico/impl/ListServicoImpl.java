...
public class ListServicoImpl {
	...
	public ListServicoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Servico.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.LIST_SERVICO, //
	...
	*/
	public Servico.PagedResults listServico(Servico.PageRequest input, long timeout) {
		...
		Servico.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Servico.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Servico.PagedResults> listServicoRequest(Servico.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Servico.PagedResults.class);
	}
