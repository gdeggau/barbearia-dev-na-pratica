 ...
 * 
 */
public class ImportServicoImpl {
	...
	public ImportServicoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportServicoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_SERVICO, //
	...
	*/
	public ImportServicoOutput importServico(ImportServicoInput input, long timeout) {
		...
		ImportServicoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportServicoOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportServicoOutput> importServicoRequest(ImportServicoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportServicoOutput.class);
	}
