...
public class TotalLucroImpl {
	...
	public TotalLucroImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(TotalLucroInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.TOTAL_LUCRO, //
	...
	*/
	public TotalLucroOutput totalLucro(TotalLucroInput input, long timeout) {
		...
		TotalLucroOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), TotalLucroOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<TotalLucroOutput> totalLucroRequest(TotalLucroInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, TotalLucroOutput.class);
	}
