...
public class GravarFotoImpl {
	...
	public GravarFotoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(GravarFotoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.GRAVAR_FOTO, //
	...
	*/
	public GravarFotoOutput gravarFoto(GravarFotoInput input, long timeout) {
		...
		GravarFotoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), GravarFotoOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<GravarFotoOutput> gravarFotoRequest(GravarFotoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, GravarFotoOutput.class);
	}
