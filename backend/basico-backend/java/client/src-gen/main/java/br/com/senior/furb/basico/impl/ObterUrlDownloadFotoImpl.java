 ...
 * 
 */
public class ObterUrlDownloadFotoImpl {
	...
	public ObterUrlDownloadFotoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ObterUrlDownloadFotoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.OBTER_URL_DOWNLOAD_FOTO, //
	...
	*/
	public ObterUrlDownloadFotoOutput obterUrlDownloadFoto(ObterUrlDownloadFotoInput input, long timeout) {
		...
		ObterUrlDownloadFotoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ObterUrlDownloadFotoOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ObterUrlDownloadFotoOutput> obterUrlDownloadFotoRequest(ObterUrlDownloadFotoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ObterUrlDownloadFotoOutput.class);
	}
