 ...
 * 
 */
public class ImportFreguesImpl {
	...
	public ImportFreguesImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportFreguesInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_FREGUES, //
	...
	*/
	public ImportFreguesOutput importFregues(ImportFreguesInput input, long timeout) {
		...
		ImportFreguesOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportFreguesOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportFreguesOutput> importFreguesRequest(ImportFreguesInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportFreguesOutput.class);
	}
